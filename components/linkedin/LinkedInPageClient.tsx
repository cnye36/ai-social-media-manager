...
import { LinkedInIcon } from '@/components/ui/channel-icons'
...
type LinkedInVoice = 'personal' | 'company'

interface LinkedInPageClientProps {
  companyId: string
...
function buildAdditionalContext(voice: LinkedInVoice, userContext: string): string {
...
function LinkedInHistory({ companyId, refreshKey }: LinkedInHistoryProps) {
  ...
  const fetchPosts = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/posts?companyId=${companyId}&channel=linkedin`)
  ...
  const voice = (post: Post): LinkedInVoice | null => {
    const v = post.generation_params?.linkedin_voice
    ...
    <div className="mt-12">
      ...
      {!loading && posts.length === 0 && !error && (
        <div className="text-center py-12 border border-dashed border-zinc-800 rounded-xl">
          <LinkedInIcon className="w-8 h-8 text-zinc-700 mx-auto mb-3" />
          <p className="text-sm text-zinc-500">No LinkedIn posts yet</p>
...
export function LinkedInPageClient({ companyId, brandColors }: LinkedInPageClientProps) {
  const [voice, setVoice] = useState<LinkedInVoice>('personal')
  ...
  const [activeVoice, setActiveVoice] = useState<LinkedInVoice>('personal')
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-8">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-blue-600/15 border border-blue-600/25 flex items-center justify-center">
              <LinkedInIcon className="w-4.5 h-4.5 text-blue-400" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-white">LinkedIn</h1>
              <p className="text-zinc-500 text-sm mt-0.5">
                {voice === 'personal'
                  ? 'Personal profile — first-person, story-driven, founder voice'
                  : 'Company page — brand voice, "we" perspective, business outcomes'}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
            <form onSubmit={handleGenerate} className="space-y-5">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="li-topic">What to write about</Label>
                <IdeaSpark
                  companyId={companyId}
                ...
                <Textarea
                  ...
                  placeholder="e.g. Why most AI implementations fail in the first 90 days"
                  rows={3}
                  required
                />
              </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    Generate {voice === 'personal' ? 'personal' : 'company'} post
                  </>
                )}
              </Button>
          ...
          <div className="sticky top-8 space-y-4">
            <PostPreview
              ...
              onReset={handleReset}
              generationParams={{ linkedin_voice: activeVoice }}
        ...
        {/* ─── History ────────────────────────────────────────────────────── */}
        <LinkedInHistory companyId={companyId} refreshKey={historyKey} />
      </div>
    </div>
  )
}
